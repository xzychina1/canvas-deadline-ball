...
**Windows** · ⬇️ [下载最新版](../../releases/latest) · 🛟 [安装说明](INSTALL.zh-CN.md) · 🔒 [隐私](PRIVACY.zh-CN.md)

<!-- TODO: 这里放一个 5-10 秒的演示 GIF(docs/demo.gif)—— 访客第一眼看的就是它 -->
...
目前**仅支持 Windows**。去 [**Releases 页面**](../../releases/latest) 拿安装版或绿色 `.exe`。

> 首次运行,Windows SmartScreen 会弹蓝色的 *"Windows 已保护你的电脑"* 警告,因为应用没做代码签名 —— 小型独立应用都这样。点 **更多信息 → 仍要运行**(只需一次)。完整步骤 + 怎么自查安全:**[INSTALL.zh-CN.md](INSTALL.zh-CN.md)**。

一切都在本地跑 —— 没账号、没服务器、什么都不上传。见 **[隐私说明](PRIVACY.zh-CN.md)**。
...
1. **拿一条日历订阅(ICS)链接。**
   ...
   > ⚠️ 把链接当密码看待 —— 拿到的人就能看你日历,泄了可以重置。(它只存在你本机 —— 见[隐私说明](PRIVACY.zh-CN.md)。)
...
- **临近 ddl 提醒** —— 比如到期前 ~1 小时提个醒。我故意先压着:quiz 写到一半被弹窗打断,正是会让人卸载的那种体验,所以*如果*做,也会是**温和、可选、默认关**的通知,绝不是抢焦点的对话框。**想要就说一声。**

## 反馈 / Issues

碰到 bug、配置卡住,或者想要 **macOS / Linux** 版、想加某个还不支持的日历源?App 里就有个 **💬 反馈** 按钮(设置面板 + 使用说明里),或者**来提 [issue](../../issues)** —— 使用上的问题、"求支持 XX 平台"的期待都很欢迎。这是个人小项目,大家提什么,接下来多半就做什么。
